...
export async function importDatabase(json: unknown): Promise<void> {
  await jsonFetch<void>(`${BASE_URL}/database/load-file`, {
    method: "POST",
...
export async function getTableSchema(name: string): Promise<FieldSchema[]> {
  return jsonFetch<FieldSchema[]>(
    `${BASE_URL}/tables/${encodeURIComponent(name)}/schema`,
  );
}
...
export async function createTable(
  ...
  await jsonFetch<void>(`${BASE_URL}/tables`, {
    method: "POST",
...
export async function getTableRecords(name: string): Promise<TableRecord[]> {
  return jsonFetch<TableRecord[]>(
    `${BASE_URL}/tables/${encodeURIComponent(name)}/records`,
  );
}
...
export async function addRecord(
  ...
  await jsonFetch<void>(`${BASE_URL}/tables/${encodeURIComponent(tableName)}/records`, {
    method: "POST",
...
export async function sortTable(
  ...
  await jsonFetch<void>(`${BASE_URL}/tables/${encodeURIComponent(tableName)}/sort`, {
    method: "POST",
...
export async function filterTable(
  ...
  return jsonFetch<TableRecord[]>(
    `${BASE_URL}/tables/${encodeURIComponent(tableName)}/filter`,
    {
      method: "POST",
...
export async function saveDatabase(): Promise<void> {
  await jsonFetch<void>(`${BASE_URL}/database/save`, { method: "POST" });
}
